...
test_that("default config.yml is applied correctly", {
  x <- suppressWarnings(generate_workbook(as_aftable(demo_df),
                                          config_path = paste0(testthat::test_path(), "/test_config.yml"),
                                          config_name = "default"))
  ...
  expect_equal(y["keywords"], c("keywords" = "example, demonstration, config.yml"))
...
test_that("minimum config.yml is applied correctly", {
  x <- suppressWarnings(generate_workbook(as_aftable(demo_df),
                                          config_path = paste0(testthat::test_path(), "/test_config.yml"),
                                          config_name = "minimum"))
...
test_that("minimum properties are applied correctly via arguments", {
  ...
  x <- suppressWarnings(generate_workbook(as_aftable(demo_df),
                                          ...
                                          keywords =  c("keywords" = "example, demonstration, config.yml")))
