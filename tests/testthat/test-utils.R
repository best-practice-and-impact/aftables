...
test_that(".determine functions correctly identify cells", {
  ...
  expect_equal(table_numeric_cells,
               as.matrix(data.frame(notes = rep(FALSE, 4),
                                    numbers = rep(TRUE, 4),
                                    currencies = rep(FALSE, 4),
                                    empty = rep(FALSE, 4),
  ...
  expect_equal(table_currency_cells,
               as.matrix(data.frame(notes = rep(FALSE, 4),
                                    numbers = rep(FALSE, 4),
...
test_that("table cleaning functions work as intended", {
  ...
  expect_equal(clean_functions_df$currencies,
               c("1.1", "1.1", "1.3", "1.2"))

  expect_equal(clean_functions_df$mixed_currencies,
               c("1.1", "1.2", "1.3", "[note]"))
  ...
  expect_equal(clean_functions_df$numbers,
               c(1.1, 1.2, 1.3, 1.4))
  expect_equal(clean_functions_df$currencies,
               c(1.1, 1.1, 1.3, 1.2))

  # numbers in mixed columns are still characters
  expect_equal(clean_functions_df$mixed_numeric,
               c("1.1", "1.2", "1.3", "[note]"))
