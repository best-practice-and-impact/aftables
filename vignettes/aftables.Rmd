...
The package depends on [{openxlsx2}](https://janmarvin.github.io/openxlsx2/), [{pillar}](https://pillar.r-lib.org/), [{stringr}](https://stringr.tidyverse.org/), [{purrr}](https://purrr.tidyverse.org/), [{tidyselect}](https://tidyselect.r-lib.org/) and [{rlang}](https://rlang.r-lib.org/), which are also installed with {aftables}.
...
At time of writing (v2.0.0) you might want to address manually some other [accessibility requirements](https://analysisfunction.civilservice.gov.uk/policy-store/releasing-statistics-in-spreadsheets/) that are not yet covered by the package such as converting the output to an ODS file rather than the proprietary xlsx format.
