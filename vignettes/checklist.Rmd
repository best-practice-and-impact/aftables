...
The assessments in the table below are based on generating an unedited output from a typical {aftables} workflow, i.e. use `create_aftable()` to make an aftable that's passed to `generate_workbook()` and then written with `openxlsx2::wb_save()`.
...
| Wrap text within cells | Yes | Met | `generate_workbook()` wraps content by default; there's a simple check in {aftables} to widen a column if it contains long strings. Users can control text wrapping by using a config file. |
...
| Columns should be a sensible width | No | Partly | {aftables} uses a fixed default column width for data tables (16), which is doubled (32) if the length of the content exceeds a threshold number of characters (50). Users can set the desired column widths and threshold number of characters by using a config file. |
...
| Add document information | Yes | Met | {aftables} supports setting document properties by providing arguments or a config file to `generate_workbook()` |
